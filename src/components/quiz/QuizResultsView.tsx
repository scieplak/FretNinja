  ...
  return (
    <div className="space-y-8">
      <header className="space-y-3">
        ...
        <p className="text-sm text-slate-300">{scoreMessage(results.score, results.total)}</p>
      </header>
      ...
      <section className="rounded-2xl border border-white/10 bg-white/5 p-6">
        ...
        <div className="mt-4 space-y-3 text-sm text-slate-300">
          {missedQuestions.length === 0 ? (
            <p>You nailed every question. Nice work!</p>
          ) : (
            missedQuestions.map((answer) => (
              <div key={answer.questionNumber} className="rounded-lg border border-white/10 bg-slate-950/60 px-4 py-3">
      ...
      <section className="flex flex-wrap items-center gap-4">
        ...
        <a href="/quiz" className="text-sm text-slate-300 hover:text-emerald-200">
          Try different mode
        </a>
