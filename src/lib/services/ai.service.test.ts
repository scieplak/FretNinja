...
describe("AIService", () => {
  ...
  describe("aggregateErrorPatterns", () => {
    ...
    it("should return empty objects for empty input", () => {
      // Arrange
      const errors: { fret_position: number | null; string_number: number | null; target_note: string | null }[] = [];
  ...
  describe("buildPersonalizedTipsPrompt", () => {
    ...
    it("should sort positions by error count descending", () => {
      ...
      // Assert
      const positionSection = result
        .split("Most frequent error positions:")[1]
        .split("Most frequently missed notes:")[0];
    ...
    it("should limit positions to top 10", () => {
      // Arrange
      const patterns = {
        byPosition: Object.fromEntries(Array.from({ length: 15 }, (_, i) => [`fret${i}-string1`, 15 - i])),
        byNote: {},
        byString: {},
      };
